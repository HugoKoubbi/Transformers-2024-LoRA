...
#n_tilded=5
#n=5
#x0_tilded=np.array([[epsilon,1],[epsilon+e,-1],[0,0.5],[e,-0.5],[0.1,-0.2]])
num_steps = int(T/dt)+1
integration_time = np.linspace(0, T, num_steps)
...
if choice == "LoRA":
    lbda=0.5
    mu=0.4
    #u=np.random.rand(d,1)/np.sqrt(d)
    #v=np.random.rand(d,1)/np.sqrt(d)
    A1 = np.eye(d)
    #A2 = np.eye(d)+lbda*np.dot(u,u.T)
    V = np.random.rand(d, d)/2
    eigenvalues, eigenvectors = np.linalg.eig(V)
    index_max = np.argmin(eigenvalues)
    v=eigenvectors[index_max]
    V1 = np.abs(V)  
    V2 = V+mu*np.dot(v,v.T)
    V2 = np.abs(V2)                # Perron Frobenius
    print("Eigenvalues of V (original):")
    print(np.linalg.eigvals(V1))
    print("Eigenvalues of V (LoRA):")
    print(np.linalg.eigvals(V2))
    # Example in paper (2d):
    # V = np.array([[0.64709742, 0.81911926],
    #                0.61210449, 0.63263484]])
    # Example in paper (3d; Re(l)<0):
...
if choice == "1rankattention":
    A1 = np.eye(d)
    norm=1
    x=np.array([norm,0,0]).reshape(3,1)
    A2 = np.dot(x,x.T)
    V1 = np.eye(d) 
    #dir_path = './Th41'
    color = '#3a4cc1'   
...
# The trajectories
#choice="polytope"
z = transformer(T, dt, n, d, A1, V1, x0)
z_tilted=transformer(T, dt, n, d, A1, V2, x0)
#z2= transformer(T, dt, n, d, A2, V1, x0)

# Set the directory path where you want to save the pictures
dir_path='pictures/V_exp'
...
visuals(d, dt, z_tilted, integration_time, conv, color, show_polytope, movie, dir_path, base_filename)
...
#visuals(d, dt, z2, integration_time, conv, color, show_polytope, movie, dir_path, base_filename)
